...
async def chat(
    ...
    log_event("pagani.api", "chat_request", user_id=username,
    ...
    try:
        ...
        if not context_docs:
            _track_analytics("failed_query", user_id=username,
                             metadata={"question": body.question[:100]})
...
async def chat_debug(
    ...
    logger.info(
        f"Debug chat request | user={username} | role={user_role} | q='{body.question[:80]}'")
...
async def chat_stream(
    ...
    logger.info(
        f"Stream chat request | user={username} | role={user_role} | q='{body.question[:80]}'")
...
async def v1_change_user_role(
    ...
    if body.new_role not in VALID_ROLES:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Invalid role. Must be one of: {valid}")
...
async def v1_delete_document(
    ...
    try:
        ...
        vs_deleted = vector_store.delete_document(doc_id) if hasattr(
            vector_store, "delete_document") else False

        # Fallback: if doc_id was a filename, try that too
        if not vs_deleted and doc:
            vs_deleted = vector_store.delete_document(
                doc.filename) if hasattr(
                vector_store, "delete_document") else False

        if not db_deleted and not vs_deleted:
            raise HTTPException(status_code=404,
                                detail="Document not found in database or vector store")
...
async def v1_list_document_versions(
    ...
    versions = db.query(DocumentVersion).filter(
        ...
        DocumentVersion.version_number.desc()).all()
    # Format for JSON
    formatted = []
    for v in versions:
        formatted.append({
...
async def v1_get_document_content(
    ...
    if doc:
        if not doc.file_path:
            ...
            raise HTTPException(status_code=404,
                                detail="No file path associated with this document")

        if not os.path.exists(doc.file_path):
            logger.warning(
                f"Physical file missing for doc {doc_id}: "
                f"{doc.file_path}. Attempting VectorStore fallback...")
            ...
            if is_temp:
                detail = (
                    f"Document '{doc.filename}' was stored in"
                    " temporary storage and has been cleared"
                    " by the system. Please re-upload it."
                )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to read document content: {e}")
...
async def v1_review_queue(
    ...
    try:
        ...
        with get_db_session() as db:
            rows = db.query(RQ).filter(
                RQ.status == "pending_review").order_by(
                RQ.created_at.desc()).all()
            items = [{"id": r.id,
                      "username": r.username,
                      "question": r.question,
                      "reason": r.reason,
...
async def v1_admin_strategist_reports(
    ...
    try:
        ...
        with get_db_session() as db:
            rows = db.query(StrategistReport).order_by(
                StrategistReport.created_at.desc()).limit(limit).all()
            return {"reports": [{
                "id": r.id,
    ...
    try:
        ...
        if result == "no_queries":
            raise HTTPException(status_code=400,
                                detail="No pending queries in the review queue to analyze.")
...
async def v1_list_golden_answers(
    ...
    try:
        ...
        from models import GoldenAnswer
        with get_db_session() as db:
            rows = db.query(GoldenAnswer).order_by(
                GoldenAnswer.created_at.desc()).limit(limit).all()
            return {"golden_answers": [
                {"id": r.id, "query": r.query, "expected_answer": r.expected_answer, "tags": r.tags} for r in rows], "total": len(rows)}
    except Exception as e:
...
async def v1_evaluations_summary():
    ...
    try:
        ...
        return {"avg_faithfulness": 0, "avg_relevance": 0, "avg_completeness": 0, "avg_f1": 0,
                "avg_latency_ms": 0, "avg_cost_usd": 0, "total_queries": 0, "queries_by_day": []}
...
async def v1_sse_publish(
    ...
    return {"published": True, "channel": channel,
            "subscribers": sse_manager.active_connections(channel)}
