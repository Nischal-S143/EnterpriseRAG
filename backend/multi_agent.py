    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in str(e) and attempt < max_retries - 1:
                ...
                logger.warning(f"429 Quota Exceeded in multi-agent. Retrying in {wait_time}s... (Attempt {attempt+1}/{max_retries})")
...
class SynthesisAgent:
    ...
    async def execute(self, state: SharedState, sse_queue: asyncio.Queue = None) -> SharedState:
        ...
        try:
            ...
            state["final_response"] = f"Error generating response: {str(e)[:100]}. Please check your API quota."
            
        if sse_queue:
            await sse_queue.put({"event": "agent", "data": {"name": "Synthesis Agent", "status": "done", "duration_ms": int((time.time() - t0) * 1000)}})
        return state

async def run_multi_agent(query: str, retrieved_chunks: list[dict], state: dict, sse_queue: asyncio.Queue):
...
async def run_single_agent(query: str, retrieved_chunks: list[dict], sse_queue: asyncio.Queue, metadata: dict = None):
