...
import rag_pipeline
from rag_pipeline import Planner, ToolExecution, ConditionalRouter, _add_to_history, _get_history
import numpy as np
...
async def test_planner_complex():
    ...
    mock_response = MockResponse(
        '{"strategy": "multi_hop", "sub_queries": ["q1", "q2"], "complexity": "high"}')
...
async def test_tool_execution_multi_hop():
    ...
    mock_vs.index.search.return_value = (
        np.array([[0.9]], dtype=np.float32), np.array([[0]], dtype=np.int64))
    mock_vs.bm25_index.get_scores.return_value = np.array([0.5], dtype=np.float32)

    executor = ToolExecution(mock_vs)
    plan = {"strategy": "multi_hop", "sub_queries": ["q1", "q2"]}

    with patch.object(executor, "_embed_query", AsyncMock(return_value=np.array([0.1] * 1536, dtype=np.float32))):
...
async def test_agentic_router():
    ...
    mock_response = MockResponse('{"needs_search": true, "search_query": "optimized query"}')
...
async def test_generate_response_stream():
    ...
    async def mock_stream():
        # Mocking the complex delta structure
        yield MagicMock(choices=[MagicMock(delta=MagicMock(delta=MagicMock(content="Part 1")))])
