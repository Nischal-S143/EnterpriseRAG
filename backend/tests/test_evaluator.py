...
def test_llm_judge_score(mock_create):
    ...
    mock_response.choices = [
        MagicMock(message=MagicMock(content='{"relevance": 9, "faithfulness": 8, "completeness": 9, "overall": 9, "reasoning": "Good"}'))
    ]
...
def test_evaluator_full(mock_create, mock_store):
    mock_response = MagicMock()
    mock_response.choices = [
        MagicMock(message=MagicMock(content='{"relevance": 10, "faithfulness": 10, "completeness": 10, "overall": 10, "reasoning": "Perfect"}'))
    ]
    mock_create.return_value = mock_response
